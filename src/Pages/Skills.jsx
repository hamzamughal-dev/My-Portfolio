...
const skills = [
  { name: "JavaScript", level: "Expert", percent: 94, icon: <FaJs size={40} color="#f7df1e" /> },
  ...
  { name: "C++", level: "Intermediate", percent: 65, icon: <FaCode size={40} color="#00599C" /> },
];
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        ...
        if (newProgress[hoveredIndex] < skills[hoveredIndex].percent) {
          newProgress[hoveredIndex] += 5; // faster increment
          if (newProgress[hoveredIndex] > skills[hoveredIndex].percent) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-black text-white px-6 py-12">
      <main className="flex-1 flex flex-col items-center justify-start max-w-6xl mx-auto">
        <h1 className="text-5xl sm:text-6xl font-extrabold mb-12 bg-clip-text text-transparent bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 animate-pulse pt-32 pb-2">
          My Skills
        </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-12 w-full">
          {skills.map((skill, idx) => (
            ...
            >
              ...
              {/* Progress Bar */}
              <div className={`w-full h-4 bg-gray-800 rounded-full overflow-hidden transition-all duration-500 opacity-0 ${hoveredIndex === idx ? "opacity-100" : ""}`}>
                <div
                  className="h-full bg-gradient-to-r from-blue-400 via-blue-600 to-blue-700"
                  style={{ width: `${progress[idx]}%`, transition: "width 0.1s" }}
                ></div>
              </div>
              {hoveredIndex === idx && <p className="text-gray-300 mt-1">{progress[idx]}%</p>}
            </div>
          ))}
        </div>
      </main>
